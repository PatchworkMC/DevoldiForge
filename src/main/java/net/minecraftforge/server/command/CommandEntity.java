...
class CommandEntity
{
    ...
    private static class EntityListCommand
    {
        ...
        static ArgumentBuilder<ServerCommandSource, ?> register()
        {
            return CommandManager.literal("list")
                ...
                .then(CommandManager.argument("filter", StringArgumentType.string())
                    .suggests((ctx, builder) -> CommandSource.suggestMatching(ForgeRegistries.ENTITIES.getKeys().stream().map(Identifier::toString), builder))
                    .then(CommandManager.argument("dim", DimensionArgumentType.dimension())
                        .executes(ctx -> execute(ctx.getSource(), StringArgumentType.getString(ctx, "filter"), DimensionArgumentType.getDimensionArgument(ctx, "dim")))
                    )
        ...
        private static int execute(ServerCommandSource sender, String filter, DimensionType dim) throws CommandSyntaxException
        {
            ...
            ServerWorld world = DimensionManager.getWorld(sender.getMinecraftServer(), dim, false, false);
            if (world == null)
