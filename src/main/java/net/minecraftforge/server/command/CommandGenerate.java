...
class CommandGenerate
{
        ...
        return CommandManager.literal("generate")
            ...
            .then(CommandManager.argument("pos", BlockPosArgumentType.blockPos())
                .then(CommandManager.argument("count", IntegerArgumentType.integer(1))
                    ...
                    .executes(ctx -> execute(ctx.getSource(), BlockPosArgumentType.getBlockPos(ctx, "pos"), getInt(ctx, "count"), ctx.getSource().getWorld().dimension.getType(), -1))
                )
            );
    }
    ...
    private static int execute(ServerCommandSource source, BlockPos pos, int count, DimensionType dim, int interval) throws CommandException
    {
