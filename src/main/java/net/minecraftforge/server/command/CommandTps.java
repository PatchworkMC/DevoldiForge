...
class CommandTps
{
    ...
    private static int sendTime(ServerCommandSource cs, DimensionType dim) throws CommandSyntaxException
    {
        long[] times = cs.getMinecraftServer().getTickTime(dim);
        ...
        cs.sendFeedback(new TranslatableText("commands.forge.tps.summary.named", dim.getRawId(), DimensionType.getId(dim), TIME_FORMATTER.format(worldTickTime), TIME_FORMATTER.format(worldTPS)), false);
