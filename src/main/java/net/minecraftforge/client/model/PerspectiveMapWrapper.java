...
public class PerspectiveMapWrapper implements IDynamicBakedModel
{
    ...
    private final ImmutableMap<ModelTransformation.Mode, Rotation3> transforms;
    private final OverrideListWrapper overrides = new OverrideListWrapper();

    public PerspectiveMapWrapper(BakedModel parent, ImmutableMap<ModelTransformation.Mode, Rotation3> transforms)
    {
        this.parent = parent;
    ...
    public static ImmutableMap<ModelTransformation.Mode, Rotation3> getTransforms(ModelBakeSettings state)
    {
        EnumMap<ModelTransformation.Mode, Rotation3> map = new EnumMap<>(ModelTransformation.Mode.class);
        for(ModelTransformation.Mode type : ModelTransformation.Mode.values())
        {
            Rotation3 tr = state.getPartTransformation(type);
            if(!tr.isIdentity())
            {
                map.put(type, tr);
            }
    ...
    public static ImmutableMap<ModelTransformation.Mode, Rotation3> getTransforms(ModelTransformation transforms)
    {
        EnumMap<ModelTransformation.Mode, Rotation3> map = new EnumMap<>(ModelTransformation.Mode.class);
        for(ModelTransformation.Mode type : ModelTransformation.Mode.values())
        {
    ...
    @SuppressWarnings("deprecation")
    public static ImmutableMap<ModelTransformation.Mode, Rotation3> getTransformsWithFallback(ModelBakeSettings state, ModelTransformation transforms)
    {
        EnumMap<ModelTransformation.Mode, Rotation3> map = new EnumMap<>(ModelTransformation.Mode.class);
        for(ModelTransformation.Mode type : ModelTransformation.Mode.values())
        {
    ...
    public static BakedModel handlePerspective(BakedModel model, ImmutableMap<ModelTransformation.Mode, Rotation3> transforms, ModelTransformation.Mode cameraTransformType, MatrixStack mat)
    {
        Rotation3 tr = transforms.getOrDefault(cameraTransformType, Rotation3.identity());
        if (!tr.isIdentity())
        {
            tr.push(mat);
        }
    ...
    {
        Rotation3 tr = state.getPartTransformation(cameraTransformType);
        if (!tr.isIdentity())
        {
            tr.push(mat);
        }
